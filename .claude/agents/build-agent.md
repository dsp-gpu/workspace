---
name: build-agent
description: Собирает репо DSP-GPU поэтапно через cmake --preset debian-local-dev. Начинает с core, потом подключает к DSP мета-репо. Анализирует ошибки сборки и исправляет их. Запускать ПОСЛЕ fix-agent.
...
║  🔴 Изменения — ТОЛЬКО после явного OK от Alex       ║
║  ✅ Разрешено: путь файла в target_sources (если     ║
║     файл реально существует по новому пути)          ║
╚═══════════════════════════════════════════════════════╝
```
...
- Перед сохранением build-лога проверить что он не содержит токенов/паролей
