...
### 7.1. Windows или Linux, и почему

Это не вопрос предпочтений. Технически серверный стек локальных LLM в 2026 году рассчитан на Linux. Ниже — ключевые причины.

**1. Серверные программы инференса выпускаются под Linux first.**
vLLM, SGLang, TensorRT-LLM — все production-релизы только Linux. Ollama на Windows работает, но без серверных функций (continuous batching, multi-user). Text Generation Inference от HuggingFace — Linux only. Запуск этих программ на Windows возможен только через WSL2 — это Linux-прослойка внутри Windows, дающая 5–15 % overhead.

**2. Поддержка AMD ROCm на Windows крайне ограничена.**
Серверные карты AMD Instinct (MI210, MI250, MI300X, MI350) работают официально **только на Linux**. Windows-поддержка ROCm существует, но только для потребительских карт и без готовых Docker-контейнеров. Если конфигурация включает карты AMD Instinct — обсуждать Windows бессмысленно.

**3. NVIDIA CUDA работает на обеих ОС, но Linux остаётся приоритетом.**
Драйверы для Linux выходят на 2–4 недели раньше Windows-версий. Бенчмарки NVIDIA публикуются на Linux. Практические замеры LLM-инференса на RTX 4090 / 5090 показывают, что Linux даёт на 10–18 % больше токенов в секунду, чем тот же vLLM в WSL2 на Windows.

**4. Docker и контейнеризация — родная среда Linux.**
Официальные Docker-образы для LLM (NVIDIA `nvcr.io/*`, AMD `rocm/*`) — Linux-native. На Windows они работают через WSL2. Kubernetes, GPU Operators от NVIDIA и AMD — Linux only.

**5. Серверная инфраструктура 24/7 удобнее на Linux.**
systemd-сервисы, SSH, Prometheus + Grafana, Ansible — стандартный набор инструментов для удалённого администрирования и мониторинга сервера. На Windows аналоги существуют, но требуют больше усилий и не покрываются AI-сообществом.

**6. Экосистема документации и сообщества.**
Официальные гайды по vLLM, SGLang, Ollama, llama.cpp — примеры на Linux, Windows упоминается в разделе «Known limitations». 95 % вопросов-ответов на Stack Overflow по настройке LLM — Linux.

**7. Производительность файловой системы.**
Модели Hugging Face — это десятки файлов в одной папке. ext4/XFS/ZFS на Linux обрабатывают такие чтения вдвое быстрее, чем NTFS на Windows.

### 7.2. Когда Windows оправдан

**Только для клиентских рабочих мест** — IDE разработчиков (VSCode, Cursor, PyCharm), терминалы для подключения к серверу по SSH. Сам сервер LLM — Linux.

Исключение: корпоративная политика обязательного использования Windows Server и отсутствие возможности выделить Linux-компетенцию. Это не техническая, а организационная причина, и она обсуждается отдельно.

### 7.3. Какой именно Linux
...
Ubuntu 24.04 LTS — основной выбор для большинства случаев: первая платформа, под которую выходят драйверы NVIDIA и ROCm; поддержка до 2029 года; стандарт де-факто в AI-сообществе.
