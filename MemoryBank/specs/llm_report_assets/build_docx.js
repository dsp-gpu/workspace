  ...
  const children = [
    ...titleBlock(
      ...
      { date: "22 апреля 2026 г.", author: "Отдел НИОКР",
        audience: "руководство, IT-директор, специалисты", version: "1.0" }
    ),
    ...
    h1("Выводы"),
    bulletP("GLM-5.1 (754 B параметров) непригодна для локальной станции. Требует 8 × H200 либо 4–8 × MI300X. Рекомендуется исключить из тестирования."),
    bulletP("Минимально рабочая конфигурация — 2 × NVIDIA RTX 5090 (32 ГБ), суммарно 64 ГБ VRAM. Покрывает все модели кроме GLM-5.1 при трёх одновременных пользователях."),
    bulletP("Рекомендуемая конфигурация — 1 × AMD Instinct MI300X (192 ГБ HBM3). Одна карта покрывает все задачи, включая LoRA 35 B, с запасом на рост до 15 пользователей."),
    bulletP("Серверная программа обязательна vLLM либо SGLang, не Ollama. При трёх пользователях Ollama даёт задержку 24.7 с (p99), vLLM — меньше 3 с на том же железе."),
    bulletP("Операционная система — Ubuntu 24.04 LTS. Windows не подходит для серверной части."),

    h1("Сводная таблица конфигураций"),
    table({
      columns: [
        { text: "Конфигурация",    width: 2600, align: AlignmentType.LEFT  },
        { text: "VRAM, ГБ",        width: 1000, align: AlignmentType.CENTER },
        { text: "Польз.",          width: 900,  align: AlignmentType.CENTER },
        { text: "LoRA 14B",        width: 1000, align: AlignmentType.CENTER },
        { text: "LoRA 35B",        width: 1000, align: AlignmentType.CENTER },
        { text: "Комментарий",     width: 3340, align: AlignmentType.LEFT  },
      ],
      rows: [
        ["1× RTX 5060 Ti 16",          "16",        "1–2",       "нет",       "нет",       "Недостаточно для сервера"],
        ["4× RTX 5060 Ti",              "64",        "8–10",      "нет",       "нет",       "Multi-GPU без NVLink — 15-30% потерь"],
        ["4× RTX 5080",                 "64",        "10–12",     "1 карта",   "нет",       "Выше bandwidth памяти чем 5060 Ti"],
        ["4× RTX 4090 24",              "96",        "14–16",     "да",        "ограниченно","Классическая AI-workstation"],
        ["2× RTX 5090 32",              "64",        "12–14",     "да",        "ограниченно","Лучший потребительский вариант"],
        ["1× AMD MI210 64",             "64",        "12",        "да",        "одна карта","Не рекоменд. — исключён из нового vLLM"],
        ["1× AMD MI250",                "128",       "16",        "да",        "да",        "То же — устаревшая поддержка"],
        [{ text: "1× AMD MI300X 192", bold: true },
          { text: "192", bold: true }, { text: "25–30", bold: true },
          { text: "да", bold: true }, { text: "да", bold: true },
          { text: "Рекомендуется", bold: true, color: C.ok }],
    ...
    bulletP("Серверные AMD Instinct MI300X и выше имеют прямую поддержку ROCm 7.0, vLLM, SGLang в виде официальных Docker-контейнеров."),
    ...
    h1("Финальная рекомендация"),
    p("Сервер: 4U rack-mount на Supermicro AS-8125GS-TNMR2 или Dell PowerEdge XE9680."),
    p("GPU: 1 × AMD Instinct MI300X 192 ГБ HBM3 (опционально расширение до 2 карт)."),
    p("CPU: 2 × AMD EPYC 9354 (64 физических ядра суммарно)."),
    p("RAM: 512 ГБ DDR5 ECC."),
    p("Диски: 2 × 3.84 ТБ NVMe (система, RAID 1) + 4 × 7.68 ТБ NVMe (модели, RAID 10)."),
    p("Сеть: 2 × 25 GbE."),
    p("ОС: Ubuntu 24.04 LTS."),
    p("Программный стек: ROCm 7.0 + vLLM 0.9+ (официальный контейнер AMD rocm/vllm:rocm7.0_vllm_mi300x)."),
  ];
