...
### 9.1. Почему Linux, а не Windows

Это не вопрос вкуса. В 2026 году серверный стек локальных LLM технически рассчитан на Linux и только частично — на Windows. Ниже восемь причин, каждая из которых — достаточная; в сумме они дают однозначный ответ.

#### 9.1.1. Программные платформы инференса выпускаются под Linux first

| Платформа | Linux | Windows нативно | Примечание |
|-----------|:-----:|:---------------:|------------|
| vLLM | Да, production | Нет | Только через WSL2 (то есть фактически Linux внутри Windows) |
| SGLang | Да, production | Нет | Linux only, WSL2 |
| TensorRT-LLM | Да, production | Preview | Production-релизы только Linux |
| Ollama | Да | Да | Windows-версия есть, но серверные фичи (continuous batching, multi-user) не реализованы на уровне Linux |
| llama.cpp | Да | Да | Кросс-платформенный, но на Windows теряется часть SIMD/AVX оптимизаций и Flash Attention |
| Text Generation Inference (HuggingFace) | Да | Нет | Linux only |

Ни одна серверная программа с continuous batching не имеет production-качества под Windows. WSL2 — это Linux-ядро внутри Windows, фактически «Linux через прослойку», дающий overhead 5–15 % против нативного Linux.

#### 9.1.2. ROCm поддержка Windows — crippled

Это критично для всех карт AMD Instinct (MI210, MI250, MI300X, MI350).

- **ROCm на Linux** — первичная платформа AMD. Полная поддержка compute + PyTorch + vLLM + SGLang + Docker-контейнеры от AMD выходят day-0.
- **ROCm на Windows** — с ROCm 5.5 (2023) появилась базовая поддержка, с ROCm 7.0 (2025) — расширенная. Но **только для потребительских карт** (RX 7000, RX 9000). Серверные AMD Instinct MI300X и MI350 на Windows **не работают** официально.
- Prebuilt vLLM Docker от AMD (`rocm/vllm:rocm7.0_vllm_mi300x`) — собираются **только под Linux**. Windows-эквивалент не выпускается.

Из этого следует: если в конфигурации выбраны AMD карты — обсуждать Windows бессмысленно.

#### 9.1.3. CUDA на обеих ОС, но Linux всё равно быстрее

NVIDIA формально поддерживает CUDA на Windows полностью. Однако:

- Бенчмарки производительности, публикуемые NVIDIA, делаются на Linux.
- Драйверы для Linux выходят раньше Windows-версий в среднем на 2–4 недели.
- CUDA на Linux имеет прямой доступ к kernel-уровню (IOMMU, PCIe peer-to-peer); на Windows часть оптимизаций недоступна из-за модели безопасности Windows Driver Framework.
- WSL2 + CUDA работает, но добавляет 3–8 % overhead через virtualization layer.

В практических тестах LLM-инференса NVIDIA RTX 4090 / 5090:
- Та же модель, тот же vLLM, одинаковые настройки.
- Linux даёт на 10–18 % больше токенов/сек, чем Windows + WSL2.

#### 9.1.4. Docker и контейнеризация

Production-деплой LLM строится на контейнерах. Здесь Linux — родная среда:

- Docker Desktop на Windows работает через WSL2 — это Linux-контейнеры внутри Linux-VM внутри Windows.
- Нативные Linux-контейнеры (NVIDIA `nvcr.io/nvidia/*`, AMD `rocm/*`) не имеют Windows-эквивалентов.
- Kubernetes, который часто используется для оркестрации нескольких LLM-сервисов, — Linux first.
- Helm charts, Operators для AI-workloads (NVIDIA GPU Operator, AMD GPU Operator) — Linux only.

Запуск контейнера на Linux — одна команда. На Windows — установка WSL2, Docker Desktop, настройка проброса GPU, дополнительные разрешения.

#### 9.1.5. Системные службы и автоматизация

Серверная установка — это не просто «запустить программу», а «программа работает 24/7, перезапускается при сбое, логируется, мониторится».

| Задача | Linux-стандарт | Windows-эквивалент |
|--------|----------------|---------------------|
| Автозапуск сервиса | systemd unit | Windows Service (сложнее настраивать для non-.NET) |
| Мониторинг процессов | Prometheus + Grafana + node_exporter | Есть аналоги, но LLM-ориентированные дашборды рассчитаны на Linux |
| GPU-мониторинг | `nvidia-smi`, `rocm-smi`, DCGM | Есть nvidia-smi; rocm-smi на Windows нет |
| Удалённое администрирование | SSH | RDP или WinRM — оба тяжелее для автоматизации |
| Конфигурация-как-код | Ansible, Terraform | Есть, но модули для AI-стека зрелее на Linux |
| Логирование | journald + rsyslog | Event Log — менее удобен для текстовых логов vLLM |

#### 9.1.6. Файловая система и загрузка моделей

Модели Hugging Face распространяются как папки с десятками файлов: один веса (`.safetensors`), несколько конфигов JSON, токенизатор, файлы авторства. Типичная модель 14 B — 15–20 файлов.

- **ext4 / XFS / ZFS** (Linux) обрабатывают множество мелких файлов быстро. Чтение 20 файлов суммарно 9 ГБ — 1–2 секунды на NVMe.
- **NTFS** (Windows) медленнее на fragmented reads из-за особенностей кеша. Та же загрузка — 3–5 секунд.

Для одной загрузки — разница несущественна. Для сервера, который перезапускается, переключает модели, — ощутимо.

#### 9.1.7. Сообщество и документация

- Официальная документация vLLM, SGLang, Ollama, llama.cpp — примеры на Linux, Windows упоминается в разделе «Known limitations».
- Stack Overflow: 95 % вопросов-ответов по настройке LLM — Linux.
- Хорошо описанные проблемы с их решениями — Linux.
- Производители железа (NVIDIA, AMD) дают тех-поддержку на Linux первым приоритетом.

При возникновении проблем на Linux — ответ находится за минуты. На Windows — часы или самостоятельное расследование.

#### 9.1.8. Когда Windows всё же имеет смысл

В рамках описываемой задачи — **никогда для сервера**. Windows имеет смысл только:

- для **клиентских рабочих мест** разработчиков — IDE (VSCode, Cursor, PyCharm, Rider), терминалы для подключения к серверу по SSH;
- для **экспериментальной установки Ollama на ноутбук разработчика** без серверной нагрузки (один пользователь, пробует модели локально);
- если в организации **жёсткая политика использовать только Windows Server** и нет возможности выделить Linux-компетенцию.

Последний случай — не техническая, а организационная причина. И она отдельно обсуждается на уровне руководства. В этом документе исходим из того, что Linux доступен.

---

### 9.2. Таблица совместимости (какой именно Linux)

После того как выбор Linux сделан, остаётся определить дистрибутив.
...
### 9.3. Требования к ядру и пакетам
