# План: 3 RAG-агента для генерации `.rag/` карточек

> **Статус**: v2 (синхронизирован с ревью v2.1) · **Создан**: 2026-05-05 · **Автор**: Кодо
>
> Цель: построить инфраструктуру и трёх агентов, которые на **8 репо** DSP-GPU (DSP пропускаем — вариант B)
> сгенерируют `<repo>/.rag/test_params/*.md`, `<repo>/.rag/use_cases/*.md`
> и `<repo>/.rag/pipelines/*.md` для расширения RAG-базы.
>
> **Цель проекта**: «слепок» кодовой базы для обучения локальной AI + семантический поиск.
>
> **v2 правки относительно v1**: vectors переехали в Qdrant `dsp_gpu_rag_v1` (Variant C из ревью), новые таблицы PG БЕЗ `embedding` колонок, DSP пропущен. Подробности — `RAG_three_agents_review_2026-05-05.md` v2.1.
>
...
## 14. Раскатка на 6 оставшихся репо (после двух pilot'ов)

После pilot'ов **spectrum** (TASK_04..08) + **strategies** (TASK_09..10) — раскатка на остальные **6 репо** (DSP пропущен — вариант B).

| Репо | Use-cases (~оценка) | Pipelines | Class-cards | Время агента+ревью |
|---|---|---|---|---|
| core | 5 | 0 | 4 | 2 ч |
| stats | 5 | 1 | 3 | 2 ч |
| signal_generators | 6 | 1 | 4 | 2 ч |
| heterodyne | 4 | 2 | 2 | 1.5 ч |
| linalg | 6 | 2 | 4 | 2.5 ч |
| radar | 4 | 3-5 | 3 | 3 ч |
| ~~DSP (мета)~~ | ~~пропущен~~ | ~~—~~ | ~~—~~ | ~~Python в C++ карточках~~ |

**Итого по 8 репо** (включая pilot'ы spectrum+strategies): ~30 use-cases + ~12 pipelines + ~25 class-cards. После каждого репо — короткое ревью Alex'ом, минимум 50% карточек `human_verified=true`.
...
### 17.1. DSP мета-репо — РЕШЕНО (вариант B)

**Решение Alex'а** (ревью v2.1 #8): **DSP пропускаем**.

Причина — в репо `DSP` нет C++ классов:
- `DSP/Python/<module>/` — Python-биндинги и тесты
- `DSP/Doc/`, `DSP/Examples/`, `DSP/Results/`, `DSP/Logs/`

Class-card агент и Pipeline агент для DSP не работают (нет C++ классов / composer'ов).

**Что вместо**: Python-API упоминается **внутри C++ use_case карточки** секцией «Python-эквивалент» + ссылка на `examples/python/<related>.py`. Образец — `use_case_fft_batch_signal.example.md` уже содержит ссылку на Python.

**Раскатка идёт на 8 репо** (без DSP): core, spectrum, stats, signal_generators, heterodyne, linalg, radar, strategies.
...
| `TASK_RAG_02_schema_migration` | DDL новых 4 таблиц + alembic stub | 1.5 | 01 |
...
| `TASK_RAG_11_rollout` | Раскатка на 7 оставшихся репо | 14 | 10 |
| `TASK_RAG_12_retrieval_validation` | R@5 на golden_set с/без HyDE | 1 | 11 |

**Правильный старт обсудим** перед тем как `01` пойдёт в работу — именно как просил Alex.

---

*План v1.1 готов. Дальше — таски + smoke PG + OK на 01.*
